...
const CONNECTORS: { id: RadarProfileConfig["enabledConnectorIds"][number]; label: string }[] = [
  ...
  { id: "official-watchlist", label: "Official Release" },
];
...
function configuration(profile: RadarProfileConfig): RadarProfileConfig {
  return {
    ...
    officialWatchlist: profile.officialWatchlist,
...
export function ProfileConfig({ connectors, mode = "settings" }: { connectors: readonly RadarConnector[]; mode?: "settings" | "sources" }) {
  ...
  const load = async () => {
    ...
    try {
      ...
      setWatchlist(JSON.stringify(editableProfile.officialWatchlist, null, 2));
  ...
  const draft = () => {
    ...
    try {
      return { ...configuration(profile), officialWatchlist: JSON.parse(watchlist) as unknown };
    } catch {
      setMessage("Official Watchlist 必须是 JSON 数组。");
      return null;
    }
  };
  ...
  const save = async () => {
    ...
    try {
      ...
      setWatchlist(JSON.stringify(result.active.officialWatchlist, null, 2));
  ...
  return <section className={`config-page ${sourceMode ? "source-monitor" : ""}`}>
    <header className="page-header config-header"><div><p className="eyeline">{sourceMode ? "Connector Health · 操作监控" : "单一 Radar Profile"}</p><h1>{sourceMode ? "管理采集来源" : "Radar Profile"}</h1><p>{sourceMode ? "加载配置后可启用、停用来源并登记 Official Watchlist；保存后在下一次 Collection Cycle 生效。" : "保存后的设置会在下一次 Worker 执行时生效。密钥始终由部署环境保管。"}</p></div>{activeProfileId ? <span className="version-select">{activeProfileId}</span> : null}</header>
