...
export function ProfileConfig({ connectors }: { connectors: readonly RadarConnector[] }) {
  ...
  const retryDelayed = async () => {
    setPending(true);
    try {
      const result = await request("/api/profile/retry-delayed", "POST") as { requeuedCount: number };
      setMessage(result.requeuedCount > 0 ? `已重新排队 ${result.requeuedCount} 个延迟 Candidate，等待 Worker 处理。` : "当前没有可重试的延迟 Candidate。");
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "重试延迟 Candidate 失败。");
    } finally {
      setPending(false);
    }
  };

  const isOllama = profile?.runtime.kind === "ollama";
  ...
  return <section className="config-page">
    ...
    <div className="config-layout"><div className="settings-form">
      ...
      {profile ? <>
        ...
        <fieldset><legend><span>02</span>筛选与采集</legend><div className="dual-fields"><label>自动采集时间<input disabled type="text" value="每天 09:00、17:00（中国标准时间）" /></label><label>每轮评估上限<input min="1" onChange={(event) => update((value) => ({ ...value, runtime: { ...value.runtime, maxAssessmentsPerCycle: Number(event.target.value) } }))} type="number" value={profile.runtime.maxAssessmentsPerCycle} /></label><label>模型并发<input min="1" onChange={(event) => update((value) => ({ ...value, runtime: { ...value.runtime, modelConcurrency: Number(event.target.value) } }))} type="number" value={profile.runtime.modelConcurrency} /></label><label>每轮预算（秒）<input min="60" onChange={(event) => update((value) => ({ ...value, runtime: { ...value.runtime, cycleBudgetSeconds: Number(event.target.value) } }))} type="number" value={profile.runtime.cycleBudgetSeconds} /></label></div><div className="dual-fields"><label>包含词（逗号分隔）<input onChange={(event) => update((value) => ({ ...value, includeTerms: event.target.value.split(",").map((term) => term.trim()).filter(Boolean) }))} value={profile.includeTerms.join(", ")} /></label><label>排除词（逗号分隔）<input onChange={(event) => update((value) => ({ ...value, excludeTerms: event.target.value.split(",").map((term) => term.trim()).filter(Boolean) }))} value={profile.excludeTerms.join(", ")} /></label></div><div className="form-actions"><button className="secondary-button" disabled={pending} onClick={() => { void collectNow(); }} type="button">立即采集</button><button className="secondary-button" disabled={pending} onClick={() => { void retryDelayed(); }} type="button">重试延迟 Candidate</button></div><p className="config-note">Worker 每天 09:00 和 17:00 自动采集并发布；手动采集只更新队列，不改写当前 Brief。重试按钮会重新排队最近 7 天内的评估延迟 Candidate。</p></fieldset>
        <fieldset><legend><span>03</span>模型运行时</legend><div className="runtime-form"><label>运行时<select onChange={(event) => update((value) => ({ ...value, runtime: { ...value.runtime, kind: event.target.value as "compatible" | "ollama" } }))} value={profile.runtime.kind}><option value="ollama">Ollama（本机运行）</option><option value="compatible">Compatible API（证据片段将发送至配置服务）</option></select></label><label>{isOllama ? "Ollama 服务地址" : "Compatible API 地址"}<input onChange={(event) => update((value) => ({ ...value, runtime: { ...value.runtime, baseUrl: event.target.value } }))} value={profile.runtime.baseUrl} /></label><label>{isOllama ? "Ollama 模型" : "Compatible API 模型"}{isOllama ? <select onChange={(event) => update((value) => ({ ...value, runtime: { ...value.runtime, model: event.target.value } }))} value={profile.runtime.model}>{models.length ? models.map((model) => <option key={model}>{model}</option>) : <option>{profile.runtime.model}</option>}</select> : <input onChange={(event) => update((value) => ({ ...value, runtime: { ...value.runtime, model: event.target.value } }))} value={profile.runtime.model} />}</label></div><div className="form-actions">{isOllama ? <button className="secondary-button" disabled={pending} onClick={() => { void testRuntime(true); }} type="button">发现 Ollama 模型</button> : null}<button className="secondary-button" disabled={pending} onClick={() => { void testRuntime(); }} type="button">真实连接测试</button></div><p className="config-note">{isOllama ? "发现模型会更新上方的 Ollama 模型列表；连接测试不会保存配置。" : "Compatible API Key 只读取部署环境；切换运行时不会覆盖当前地址或模型，请填写对应服务的 HTTPS 地址与模型后再测试。"}</p></fieldset>
