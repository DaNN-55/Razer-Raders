...
export function ProfileConfig({ connectors, mode = "settings" }: { connectors: readonly RadarConnector[]; mode?: "settings" | "sources" }) {
  ...
  return <section className={`config-page ${sourceMode ? "source-monitor" : ""}`}>
    <header className="page-header config-header"><div><p className="eyeline">{sourceMode ? "Connector Health · 操作监控" : "单一 Radar Profile"}</p><h1>{sourceMode ? "管理采集来源" : "Radar Profile"}</h1><p>{sourceMode ? "加载配置后可启用、停用来源；保存后在下一次 Collection Cycle 生效。" : "保存后的设置会在下一次 Worker 执行时生效。密钥始终由部署环境保管。"}</p></div>{activeProfileId ? <span className="version-select">{activeProfileId}</span> : null}</header>
